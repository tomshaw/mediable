<div>
    ...
    <ul class="flex flex-wrap -mx-1 p-0 pl-1 w-full">
        ...
        <li @class([
            "attachment relative flex m-0 p-0 cursor-pointer list-none text-center select-none border-b border-r transition-colors duration-200",
            ...
            <div class="relative cursor-pointer flex items-center justify-center min-w-full" style="padding: {{ $this->normalizeColumnPadding($columnWidths[$defaultColumnWidth]) }}rem;">
                ...
                <div class="absolute left-2.5 top-1 cursor-pointer bg-transparent">
                    <span @class([
                        ...
                        in_array($item->id, $selectedIds) ? "text-blue-700" : "text-neutral-600"
                    ])>{{ $item->id }}</span>
                </div>
                ...
                <img src="{{ asset($item->file_url) }}?v={{ $this->cacheKey($item->updated_at) }}" @class([
                    ...
                    in_array($item->id, $selectedIds) ? 'border-blue-500' : 'border-black'
                ...
                <div class="flex items-center justify-center h-full w-full overflow-hidden">
                    ...
                    <button wire:click.stop="playAudio({{ $item->id }})" wire:island="selection" id="playIcon{{ $item->id }}" @class(['w-12 h-12 bg-neutral-900 rounded-full items-center justify-center cursor-pointer', ($item->id === $audioElementId) ? 'hidden' : 'flex'])>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-6 h-6 text-white">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path>
                        </svg>
                    </button>

                    <button wire:click.stop="pauseAudio({{ $item->id }})" wire:island="selection" id="pauseIcon{{ $item->id }}" @class(['bg-white bg-opacity-10 w-12 h-12 justify-between items-end p-2 box-border cursor-pointer gap-x-[1px]', ($item->id === $audioElementId) ? 'flex' : 'hidden'])>
