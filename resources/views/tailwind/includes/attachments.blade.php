  ...
  <div class="overflow-hidden w-full">
    <table class="border border-collapse">
      ...
      <tbody>
        ...
        <tr @class(['selected'=> in_array($item->id, array_column($this->selected, 'id')), 'details' => $item->id == $this->modelId]) wire:click="toggleAttachment({{$item}})">
          <td class="center">
          ...
          <td>
            <button type="button" class="btn" wire:click="deleteAttachment({{$item->id}})">Trash</button>
          </td>
        </tr>
        @endforeach
      </tbody>
    ...
    <li @class([ 'relative flex m-0 p-2 cursor-pointer list-none text-center select-none w-[20%]', 'selected shadow-[inset_0_0_0_2px_#fff,_inset_0_0_0_7px_#00b5d2]'=> in_array($item->id, array_column($this->selected, 'id')), 'details' => $item->id == $this->modelId])
      ...
      <div class="relative bg-[#e5e7eb] cursor-pointer py-4 md:py-8 lg:py-12 xl:py-16 px-4 md:px-8 flex items-center justify-center min-w-full">
        ...
        <div class="relative object-contain overflow-hidden">
          ...
          <button wire:click.stop="$dispatch('audio:start', { id: {{ $item->id }} })" id="playIcon{{ $item->id }}" @class([
            'w-12 h-12 bg-[#444] rounded-full flex items-center justify-center cursor-pointer' => $item->id != $this->audioElementId, 
          ...
          <button wire:click.stop="$dispatch('audio:pause', { id: {{ $item->id }} })" id="pauseIcon{{ $item->id }}" @class([
            'bg-white bg-opacity-10 w-12 h-12 rounded-md hidden justify-between items-end p-2 box-border cursor-pointer gap-x-[1px]' => $item->id != $this->audioElementId, 
